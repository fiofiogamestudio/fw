...
| [fwv/](fwv/README.md) | FWD 美术工作台：素材、问题、候选与验收 | 本机资产工程 |

FWC、FWS 是库，入口不会自动生成游戏或安装全局技能。FWE、FWA、FWV 首次启动准备各自的 `.local/demo/`，后续启动保留已有数据；FWV 已有的资产工程继续使用。浏览器服务就绪后自动打开页面，关闭启动窗口或按 Ctrl+C 可停止服务。

FWC 是游戏基础设施，FWE 是工具基础设施，二者相互独立。FWA 和 FWD 的界面复用 FWE，各自业务内核仍通过 CLI/API 独立使用；FW 负责组件装配与启动，FWS 提供 Agent 操作技能。FWE 编辑草稿的撤销、FWA 已采用变化的撤销、FWD 资产版本选择分别由对应层负责。

美术产品使用 FWD 名称；兼容现有 `fwv/` 目录、`fwv` 命令和资产格式。`start.bat fwd` 与 `start.bat fwv` 打开同一组件和已有工程。
...
FWD 的 `fwv/` 源码随本工作台维护，目前尚未登记为独立 Git 子模块。其他四个组件分别在各自仓库提交，由顶层工作台的 gitlink 固定所采用的版本。
