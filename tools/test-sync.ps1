...
$tool = Join-Path $PSScriptRoot 'sync.ps1'
$powerShell = [System.Diagnostics.Process]::GetCurrentProcess().MainModule.FileName
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ('fw-sync-regression-' + [guid]::NewGuid().ToString('N'))
$previousProtocol = $env:GIT_ALLOW_PROTOCOL
$env:GIT_ALLOW_PROTOCOL = 'file:https:ssh'
$previousConfigCount = $env:GIT_CONFIG_COUNT
$testConfigIndex = if ($previousConfigCount) { [int]$previousConfigCount } else { 0 }
$testConfigKey = "GIT_CONFIG_KEY_$testConfigIndex"
$testConfigValue = "GIT_CONFIG_VALUE_$testConfigIndex"
$previousConfigKey = [Environment]::GetEnvironmentVariable($testConfigKey)
$previousConfigValue = [Environment]::GetEnvironmentVariable($testConfigValue)
[Environment]::SetEnvironmentVariable($testConfigKey, 'core.fsmonitor')
[Environment]::SetEnvironmentVariable($testConfigValue, 'false')
$env:GIT_CONFIG_COUNT = [string]($testConfigIndex + 1)
...
function Invoke-FwSync {
    ...
    try {
        $ErrorActionPreference = 'Continue'
        $rawOutput = & $powerShell -NoProfile -ExecutionPolicy Bypass -File $tool @ToolArguments 2>&1
        $exitCode = $LASTEXITCODE
    }
    finally {
        $ErrorActionPreference = $previousPreference
...
try {
    ...
    # Legacy FW installation paths and parameter names remain compatible.
    ...
    Assert-True -Condition (@($discoveredLegacy.Report.components | Where-Object { $_.component -eq 'fwc' -and $_.path -eq 'fw' }).Count -eq 1) -Message 'Legacy fw installation path was not discovered.'
    ...
    $fwc = New-TestRemote -Name 'fwc-pinned'
    ...
    $pinnedNew = Invoke-FwSync -ToolArguments @('new', '-ProjectRoot', $pinnedPath, '-Component', 'fwc', '-FwUrl', $fwc.Bare, '-FwTarget', $pinnedHead, '-Apply', '-Json')
    ...
    if ($env:OS -ne 'Windows_NT') { & chmod +x (Join-Path $hooks 'post-checkout') }
    ...
    if ($env:OS -ne 'Windows_NT') { & chmod +x (Join-Path $pairHooks 'post-merge') }
    ...
    # The top-level fw URL must not become an FWC alias, even without children.
    $workspaceRemote = New-TestRemote -Name 'fw'
    $workspace = Join-Path $testRoot 'top-workspace'
    Invoke-TestGit -WorkingDirectory $testRoot -GitArguments @('clone', $workspaceRemote.Bare, $workspace) | Out-Null
    $emptyWorkspace = Invoke-FwSync -ToolArguments @('status', '-ProjectRoot', $workspace, '-Components', 'fwc', '-Json') -ExpectedExitCodes @(2)
    Assert-True -Condition (@($emptyWorkspace.Report.components).Count -eq 0) -Message 'Top-level fw.git was incorrectly identified as FWC.'
    Invoke-TestGit -WorkingDirectory $workspace -GitArguments @('clone', $fw.Bare, 'fwc') | Out-Null
    Invoke-TestGit -WorkingDirectory $workspace -GitArguments @('clone', $fws.Bare, 'fws') | Out-Null
    $workspaceStatus = Invoke-FwSync -ToolArguments @('status', '-ProjectRoot', $workspace, '-Json')
    Assert-True -Condition (@($workspaceStatus.Report.components).Count -eq 2 -and @($workspaceStatus.Report.components | Where-Object { $_.path -eq '.' }).Count -eq 0) -Message 'FW workspace stole a sibling component identity.'
    $workspaceAsFwc = Invoke-FwSync -ToolArguments @('new', '-ProjectRoot', (Join-Path $testRoot 'wrong-source-host'), '-Component', 'fwc', '-FwcUrl', $workspaceRemote.Bare, '-Apply', '-Json') -ExpectedExitCodes @(2)
    Assert-True -Condition (-not $workspaceAsFwc.Report.applied) -Message 'New accepted fw.git as a component source.'

    # sync restores HEAD's pin; it does not upgrade to an available remote tip.
    Invoke-TestGit -WorkingDirectory $pinnedPath -GitArguments @('config', 'user.email', 'fw-sync@test.local') | Out-Null
    Invoke-TestGit -WorkingDirectory $pinnedPath -GitArguments @('config', 'user.name', 'FW Sync Test') | Out-Null
    ...
    Assert-True -Condition ((Invoke-TestGit -WorkingDirectory $pinnedRepo -GitArguments @('rev-parse', 'HEAD')).Text -eq $newerHead -and -not $syncPlan.Report.applied) -Message 'sync preview moved the component.'
    ...
    Assert-True -Condition (-not $dirtySync.Report.applied -and (Test-Path -LiteralPath $syncDirtyPath)) -Message 'sync overwrote dirty component work.'
    Remove-Item -LiteralPath $syncDirtyPath
    Add-Content -LiteralPath (Join-Path $pinnedPath '.gitmodules') -Value '# uncommitted module configuration'
    $modulesSync = Invoke-FwSync -ToolArguments @('sync', '-ProjectRoot', $pinnedPath, '-Apply', '-Json') -ExpectedExitCodes @(2)
    Assert-True -Condition (-not $modulesSync.Report.applied) -Message 'sync accepted an uncommitted .gitmodules change.'
    $syncClonePath = Join-Path $testRoot 'pinned-sync-clone'
    Invoke-TestGit -WorkingDirectory $testRoot -GitArguments @('clone', '--no-recurse-submodules', $pinnedPath, $syncClonePath) | Out-Null
    $cloneSync = Invoke-FwSync -ToolArguments @('sync', '-ProjectRoot', $syncClonePath, '-Apply', '-Json')
    Assert-True -Condition ($cloneSync.Report.applied -and $cloneSync.Report.components[0].localHead -eq $pinnedHead) -Message 'sync did not initialize the committed host pin.'

    # Explicit CSV selection installs exactly those components, without nesting.
    $nestedHead = (Invoke-TestGit -WorkingDirectory $fwe.Seed -GitArguments @('rev-parse', 'HEAD')).Text
    Invoke-TestGit -WorkingDirectory $fwc.Seed -GitArguments @('config', '-f', '.gitmodules', 'submodule.nested-fwe.path', 'nested/fwe') | Out-Null
    Invoke-TestGit -WorkingDirectory $fwc.Seed -GitArguments @('config', '-f', '.gitmodules', 'submodule.nested-fwe.url', $fwe.Bare) | Out-Null
    Invoke-TestGit -WorkingDirectory $fwc.Seed -GitArguments @('add', '.gitmodules') | Out-Null
    Invoke-TestGit -WorkingDirectory $fwc.Seed -GitArguments @('update-index', '--add', '--cacheinfo', "160000,$nestedHead,nested/fwe") | Out-Null
    Invoke-TestGit -WorkingDirectory $fwc.Seed -GitArguments @('commit', '-m', 'nested dependency fixture') | Out-Null
    ...
    try {
        $env:FW_TEST_POWERSHELL = $powerShell
        $boundaryArgs = if ($KeepFixture) { @('--keep') } else { @() }
        $boundaryOutput = & node (Join-Path $PSScriptRoot 'test-sync-boundaries.mjs') @boundaryArgs
        if ($LASTEXITCODE -ne 0) { throw 'Additional Git boundary regressions failed.' }
        $boundaryChecks = $boundaryOutput | ConvertFrom-Json
        Assert-True -Condition $boundaryChecks.success -Message 'Additional Git boundary regressions did not report success.'
    }
    finally { $env:FW_TEST_POWERSHELL = $previousTestPowerShell }
    ...
    [pscustomobject]@{
        ...
        topFwIsNotFwc = $true
        syncUsesCommittedHeadWithoutFloatingFetch = $true
        syncRejectsUnbornStagedDirtyAndModulesDrift = $true
        syncInitializesPinnedClone = $true
        csvSelectionAndNoRecursiveComponents = $true
    ...
    [Environment]::SetEnvironmentVariable($testConfigKey, $previousConfigKey)
    [Environment]::SetEnvironmentVariable($testConfigValue, $previousConfigValue)
    if (-not $KeepFixture) {
