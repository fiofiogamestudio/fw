...
- `csharp/bridge/net`：网络 bridge 内部运行层，包括 `net_runtime`、`authority_runtime`、`client_runtime`、`udp_runtime`、`net_context`、`net_config`、`net_state`。
...
- C# core system 通过 `CoreRuntime` 推进玩法权威阶段。
- system 的 `refs` 只指向其他 system 的 context，不直接持有其他 system 本体。

## Presentation
- `FUI` 是 UI service，对外使用 `open / close`。
- `PoolManager` 是对象池 service，对外使用 `spawn / recycle / flush`。
- `AssetLoader` 是资源 service，对外使用 `load / unload`。
...
- 当前运行时数据是 Godot `Dictionary`，网络传输使用 `GD.VarToBytes`。
- 如果未来需要跨语言公共协议、长期二进制兼容、protobuf 级别的 `oneof` 约束，再引入严格 protobuf packet 层。
- `GameBridge` 用于本地单机，直接调用 `GameCore` 并返回 view/event/map 数据。
- `NetBridge` 用于联机，内部通过 `NetRuntime` 分发到 `AuthorityRuntime` 或 `ClientRuntime`。
...
## Bridge Proto
- `value.proto`：公共词典，只放跨 intent/view/event/packet 复用的基础类型和枚举。
...
- `join_accept.player_id`：host 分配给客户端的玩家 id。
